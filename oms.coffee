...
class @['OverlappingMarkerSpiderfier']
  ...
  p['spiderListener'] = (marker) ->
      ...
      if markerSpiderfied
          ...
          markerPt = @llToPt(marker.getLatLng())
          for m in @markers
              mPt = @llToPt(m.getLatLng())
  ...
  p['willSpiderfy'] = (marker) ->
      ...
      for m in @markers
          ...
          mPt = @llToPt(m['_omsData']?.usualPosition ? m.getLatLng())
  ...
  p['markersThatWillAndWontSpiderfy'] = ->  # *very* much quicker than calling willSpiderfy in a loop
      ...
      mData = for m in @markers
          {pt: @llToPt(m['_omsData']?.usualPosition ? m.getLatLng()), willSpiderfy: no}
      for m1, i1 in @markers
          continue unless m1._map?
          m1Data = mData[i1]
          continue if m1Data.willSpiderfy
          for m2, i2 in @markers
              continue if i2 is i1
              continue unless m2._map?
              m2Data = mData[i2]
  ...
  p['llToPt'] = (ll) -> @map.latLngToLayerPoint(ll)

  p['ptToLl'] = (pt) -> @map.layerPointToLatLng(pt)
